...
int main()
{
	...
	while(t--)
	{
		...
		sleft.clear(); sright.clear();
		odd.clear(); even.clear();
		for(int a=0;a<k;a++)
		{
			int i = readint(), j = readint();
			int p = min(i-1,j-1), q = min(n-i,j-1);
			sright.insert(i-p-(j-p)+n);
			sleft.insert(i+q+(j-q)-1);
		}
		LL s=0;
		for(auto i : sleft)
		{
		...
		for(auto i : sright)
		{
			int start = i <= n ? n-i+1 : i-n+1;
			int end = start + 2*((i > n ? 2*n-i : i)-1);
			vector<int> &tmp = start % 2 ? odd : even;

			int p = lower_bound(tmp.begin(), tmp.end(), end) - tmp.begin();
			if(p < tmp.size() and tmp[p] == end) s++;
			p -= (lower_bound(tmp.begin(), tmp.end(), start) - tmp.begin());
			s += p;
		}
