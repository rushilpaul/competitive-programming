...
int main()
{
	...
	while(q--)
	{
		int n = readint(), u = readint(), v = readint(), L = 0;
		if(u == v)
		{
			printf("%d\n",n);
			continue;
		...
		LL ans = 0;
		int num = n;
		string tmp = bitset<8>(num).to_string();
		int pos = -1;
		for(int a=0;a<tmp.length();a++)
			if(tmp[a] == '1') { pos = a; break; }
		if(pos >= 0)
			tmp = tmp.substr(pos);

		rep(a,L)
			tmp[tmp.length() - a - 1] = '0';

		L = suf2.length();
		rep(a,L)
		{
			tmp[tmp.length() - a - 1] = suf2[suf2.length() - a - 1];
		}
		//printf("this %s\n",tmp.c_str());

		int bits = tmp.length();
		int last = max(suf1.length(), suf2.length());

		for(int a = 0; a < bits - last; a++)
		{
			int rem = bits - last - 1 - a;
			if(tmp[a] == '1')
				ans += (1 << rem);
		}
		//printf("%s\n",tmp.c_str());
