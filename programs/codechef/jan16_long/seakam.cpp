...
void f(int i, vector< pair<int,int> > v)
{
	...
	{
		int s=0, size = v.size();
		if(size == 0) { ans = fact[n]; return; }
		sort(v.begin(), v.end(), [](pair<int,int> a, pair<int,int> b) { return a.second < b.second or (a.second == b.second and a.first < b.first); });
		rep(a,size-1)
			if(v[a].second == v[a+1].second) return;
		sort(all(v));
		rep(a,size-1)
			if(v[a].first == v[a+1].first) return;

		map<int,int> G, vis;
		rep(a,size) G[v[a].first] = v[a].second;
		int num = 0, total = 0;
		...
		ans = (ans + (size % 2 ? -1ll : 1ll) * fact[n-total+num]) % mod;
		return;
	}
	f(i+1,v);
	v.pb(edges[i]);
