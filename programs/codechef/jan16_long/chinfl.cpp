...
pair<LD,LD> dp[1001][1001];	// Rs -> Rs, Rs -> $

LD f(int i, int j, LD money, int rs)	// ith kiosk, jth second
{
	if(money > 1e18f or money < 0)
		fuckingrich = 1;
	if(i < 0 or i >= n) return 0;
	if(j == m)
	{
		dp[i][j] = { 1, ar[i][j].second };
		return rs ? money : money * dp[i][j].second;
	}

	auto &ret = dp[i][j];
	if(rs and ret.first > 0) return ret.first * money;
	if(!rs and ret.second > 0) return ret.second * money;

	// do nothing
	LD best = max(f(i+1,j+1,money,rs), f(i-1,j+1,money,rs));
	best = max(best, f(i,j+1,money,rs));
	if(rs)
	{
		best = max(best, f(i,j+1, money / ar[i][j].first, 0));
		dp[i][j].first = max((LD)1, best / money);	// Rs / Rs
	}
	else
	{
		best = max(best, f(i,j+1, money * ar[i][j].second, 1));
		dp[i][j].second = max((LD)ar[i][j].second, best / money);	// Rs / $
	}
	if(best > 1e18f) fuckingrich = 1;
	return best;
}
...
int main()
{
	...
	rep(a,n)
	{
		ans = max(ans, f(a,0,D,1));
		if(fuckingrich) break;
	}
	if(fuckingrich)
