...
vector< pair< int,pair<int,int> > > G[100001];
int bestcost[33][33];
...
int f(int i)
{
	...
	int ans = INT_MAX;
	repab(k,1,i)
		ans = min(ans,f(k-1) + bestcost[k][i]);
	return ret = ans;
}
...
int main()
{
	...
	rep(a,m)
	{
		int u = readint(), v = readint(), i = readint(), j = readint();
		G[u].pb( {v,{i,j}} );
	}
	paths.resize(G[1].size());

	rep(a,G[1].size())
	{
		auto next = G[G[1][a].first];
		paths[a].pb(G[1][a].second);

		while(next.size() == 1)
		{
			paths[a].pb(next[0].second);
			next = G[next[0].first];
		}
	}
	repab(i,1,32)
	repab(j,i,32)
	{
		bestcost[i][j] = INT_MAX;
		for(auto p : paths)
		{
			int cost = 0;
			pair<int,int> cur = {i,j};
			for(auto r : p)
				cost += max(r.first - cur.first,0) + max(cur.second - r.second,0);
			bestcost[i][j] = min(bestcost[i][j], cost);
		}
	}
