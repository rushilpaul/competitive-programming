...
int get(vector<int> &coord)
{
	int k=1,i=0;
	for(int a=n;a>=1;a--,k*=d) i += k*coord[a];
	return i;
}

LL f(vector<int> coord)
{
	int flag = 1;
	for(int a=1;a<=n;a++)
		if(coord[a] != d-1) { flag = 0; break; }
	if(flag)	// last cell
	...
	for(int a=1;a<=n;a++)
	{
		if(coord[a]+1 == d) continue;
		coord[a]++;
		ans = min(ans,f(coord));
		coord[a]--;
	}
	LL q1=0,q2=0;
	for(int a=1;a<=n;a++) { q1 += coord[a]; q2 ^= coord[a]; }
	ans += q1*q2;
	return r = ans;
}
