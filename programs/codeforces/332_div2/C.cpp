...
int overlap(pair<int,int> i, pair<int,int> j)
{
	if(j.first < i.first and j.second > i.second) return 1;
	return (j.first >= i.first and j.first <= i.second) or (j.second >= i.first and j.second <= i.second);
}
...
int main()
{
	...
	rep(a,v.size())
	{
		if(v[a].first > v[a].second)
			swap(v[a].first, v[a].second);
	}
	sort(all(v));
	/*
	rep(a,v.size())
	{
		printf("%d %d\n",v[a].first+1, v[a].second+1);
	}
	*/
	...
	{
		if(overlap(v[a], {i,j}))
			i = min(v[a].first, i), j = max(v[a].second, j);
		else
		{
			ans++;
			i = v[a].first;
