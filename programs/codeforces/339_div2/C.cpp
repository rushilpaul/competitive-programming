...
inline LD linedist(LL x1, LL y1, eq L)
{
	return abs(L.a * x1 + L.b * y1 + L.c) / sqrt(hypot(L.a, L.b));
}

inline eq line(LL x1, LL y1, LL x2, LL y2)
{
	eq L;
	L.a = y2 - y1;
	L.b = x1 - x2;
	L.c = y1 * (x2 - x1) + x1 * (y1 - y2);
	return L;
}

inline LD slope(LL x1, LL y1, LL x2, LL y2)
{
	return x1 == x2 ? LLONG_MAX : (y1 - y2) / ((LD)x1 - x2);
}

LD finddist(LL x1, LL y1, LL x2, LL y2, LL a, LL b)
{
	LD m = slope(x1,y1,x2,y2);
	if(m == LLONG_MAX)
	{
		if((b >= y1 and b <= y2) or (b >= y2 and b <= y1)) return abs(x1 - a);
		return -1;	// not in line
	}
	LD x = (b - y1 + (LD)a / m + m * x1) / (m + 1/m);
	LD y = y1 + m * (x - x1);
...
int main()
{
	...
	minR = maxR = dist(x,y,ar[0].first,ar[0].second);
	rep(a,n)
	{
		LD d = dist(x,y,ar[a].first,ar[a].second);
		maxR = max(maxR, d);
		minR = min(minR, d);
	}
	...
	rep(i,n)
	{
		LD D = finddist(ar[i].first, ar[i].second, ar[i+1].first, ar[i+1].second, x,y);
		if(D >= 0) minR = min(minR, D);
	}
	printf("%.18lf\n", (double)(maxR * maxR - minR * minR) * PI);
	return 0;
}
