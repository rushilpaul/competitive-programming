...
int f(int i, int n, int prev)
{
	...
	// for 2
	if(i+1 >= n) return ret = 0;
	string suf = s.substr(i,2);
	int possible = 0;
	if(suf != s.substr(i-prev,prev) and f(i+2,n,2))
	{
		possible = 1;
		suffix.insert(suf);
	}
	...
	// for 3
	if(i+2 >= n) return ret = possible;
	suf = suf + s[i+2];
	int possible2 = 0;
	if(suf != s.substr(i-prev,prev) and f(i+3,n,3))
	{
		suffix.insert(suf);
		possible2 = 1;
	}
	return ret = (possible or possible2);
}
