...
struct triplet
{
	...
	friend int operator==(triplet &t1, triplet &t2);
	friend int operator<(triplet &t1, triplet &t2);
};
...
int operator<(const triplet &t1, const triplet &t2)
{
	return t1.a < t2.a or (t1.a == t2.a and t1.b < t2.b) or (t1.a == t2.a and t1.b == t2.b and t1.c < t2.c);
}

set<triplet> s;
//map<triplet,LL> dp;

LL g(int a, int b, int c, int l)
{
	if(l < 0) return 0;
	auto it = s.find(triplet(a,b,c));
	if(it == s.end())
		s.insert(triplet(a,b,c));
	else
		return 0;
	LL ans = a<b+c and b<a+c and c<a+b;
	ans += g(a+1,b,c,l-1);
	ans += g(a,b+1,c,l-1);
	ans += g(a,b,c+1,l-1);
	return ans;
...
int main()
{
	int a = readint(), b = readint(), c = readint(), l = readint();
	printf("%lld\n",g(a,b,c,l));
	return 0;
}
