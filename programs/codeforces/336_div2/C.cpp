...
int main()
{
	...
	rep(a,n)
	{
		int pos = readint(), power = readint();
		v.pb({pos,power});
	}
	sort(all(v));
	int pos[100001];
	rep(a,n) pos[a] = v[a].first;

	rep(a,n)
	{
		int end = pos[a] - v[a].second;
		int i = lower_bound(pos,pos+n,end) - pos;
		D[a] = a - i;
	}
	...
	{
		int end = a - D[a] - 1;
		left1[a] = (end < 0 ? 0 : left1[end]) + D[a];
	}
	int ans = left1[n-1];
	rep(a,n)
		ans = min(ans, n-a + (a == 0 ? 0 : left1[a-1]));
	printf("%d\n",ans);
