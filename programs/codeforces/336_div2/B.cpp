...
int ones[200011], zeros[200011];

int main()
{
	int n1 = readstr(A), n2 = readstr(B);
	rep(a,n1) A[a] -= 48;
	rep(a,n2) B[a] -= 48;
	LL ans = 0;

	for(int a=n2-1;a>=0;a--)
		if(B[a] == 0) zeros[a] = zeros[a+1] + 1;
		else zeros[a] = zeros[a+1];

	for(int a=n2-1;a>=0;a--)
		if(B[a] == 1) ones[a] = ones[a+1] + 1;
		else ones[a] = ones[a+1];
	
	// zeros
	rep(a,n1)
		if(A[a] == 1)
			ans += zeros[a] - zeros[n2-(n1-a)+1];
		else
			ans += ones[a] - ones[n2-(n1-a)+1];
	cout << ans << endl;
