...
{
	LL y1 = a.second * x1 + a.first, y2 = a.second * x2 + a.second;
	LL y3 = b.second * x1 + b.first, y4 = b.second * x2 + b.second;
	return y1 < y3 or (y1 == y3 and y2 < y4);
}
...
int main()
{
	...
	rep(a,n-1)
	{
		LD x = (v[a+1].first - v[a].first) / (LD)(v[a].second - v[a+1].second);
		if(x > x1 and x < x2) flag = 1;
	}
	printf("%s\n",flag ? "YES" : "NO");
