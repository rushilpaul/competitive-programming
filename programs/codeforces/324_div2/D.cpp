...
int main()
{
	...
	rep(a,size)
	{
		repab(b,a,size-1)
		{
			int s = primes[a] + primes[b];
			ar[s] = {primes[a],primes[b]};
		}
	}
	int n = readint();
	if(f(n))
	...
	{
		if(f(ar[a].first) and f(ar[a].second) and f(n-ar[a].first-ar[a].second))
		{
			printf("3\n%d %d %d\n",n-ar[a].first-ar[a].second, ar[a].first, ar[a].second);
			return 0;
		}
		else if(f(ar[a].first) and f(n-ar[a].first))
		{
			printf("2\n%d %d\n",n-ar[a].first,ar[a].first);
			return 0;
		}
		else if(f(ar[a].second) and f(n-ar[a].second))
		{
			printf("2\n%d %d\n",n-ar[a].second,ar[a].second);
			return 0;
		}
	}
	assert(0);
