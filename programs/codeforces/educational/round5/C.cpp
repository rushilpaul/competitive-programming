...
int valid(int i, int j) { return i >= 0 and j >= 0 and i < n and j < m and s[i][j] == '.' and done[i][j] == 0; }
int lel(int i, int j) { return i >= 0 and j >= 0 and i < n and j < m and s[i][j] == '.'; }

inline int f(int i, int j, int C)
{
...
int main()
{
	...
	rep(a,n)
	{
		rep(b,m)
			...
			{
				set<int> t;
				if(lel(a-1,b)) t.insert(done[a-1][b]);
				if(lel(a+1,b)) t.insert(done[a+1][b]);
				if(lel(a,b-1)) t.insert(done[a][b-1]);
				if(lel(a,b+1)) t.insert(done[a][b+1]);
				int x = 1;
				for(int i : t)
					x += sizes[i];
