...
int main()
{
	...
	{
 		auto i = *s.begin();
		int cost = i.first, pos = i.second;
		vis[pos] = 1;
		s.erase(s.begin());
		if(pos-1 >= 0 and !vis[pos-1] and done[pos]+1 < done[pos-1])
		{
			auto it = s.find({done[pos-1],pos-1});
			assert(it != s.end());
			s.erase(it);
			done[pos-1] = min(done[pos-1], cost+1);
			s.insert({done[pos-1],pos-1});
		}
		if(pos+1 < n and !vis[pos+1] and done[pos]+1 < done[pos+1])
		{
			auto it = s.find({done[pos+1],pos+1});
			assert(it != s.end());
			s.erase(it);
