...
void Union(subset subsets[], int x, int y)
{
    ...
    int yroot = find(subsets, y);
 	min_w.erase( {subsets[xroot].w, xroot} );
	min_w.erase( {subsets[yroot].w, yroot} );

    if (subsets[xroot].rank < subsets[yroot].rank)
	{
        subsets[xroot].parent = yroot;
		subsets[yroot].w += subsets[xroot].w;
		min_w.insert( {subsets[yroot].w, yroot} );
	}
    else if (subsets[xroot].rank > subsets[yroot].rank)
	{
        subsets[yroot].parent = xroot;
		subsets[xroot].w += subsets[yroot].w;
		min_w.insert( {subsets[xroot].w, xroot} );
	}
    else
    {
        subsets[yroot].parent = xroot;
...
int main()
{
	...
	for(int a=0;a<m;a++)
	{
		...
		int x = find(subs, u), y = find(subs, v);
		if(x != y)
			Union(subs,x,y);
		printf("%d\n",min_w.begin()->first);
	}
