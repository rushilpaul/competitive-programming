...
inline node merge(node &L, node &R)
{
	node ans;
	ans.len = L.len + R.len;
	ans.n = (L.n * pow(2,R.len,3) + R.n) % 3;
	//ans.n = L.n * pow(2,R.len) + R.n;
	return ans;
}
...
void update(node *root, int i, int j, int pos)
{
	if(i == j)
	{
		root->n = 1;
	...
	int mid = (i+j)/2;
	if(pos <= mid) update(root->left,i,mid,pos);
	else update(root->right,mid+1,j,pos);
