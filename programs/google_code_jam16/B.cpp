...
int main()
{
	...
	{
		printf("Case #%d: ",t+1);
		string s1 = readstring(500), s2 = readstring(500);
		string s1_b = s1, s2_b = s2;
		int n = s1.length();
		int flag = 0;
		rep(a,n)
		{
			int diff = abs(s1[a] - s2[a]);
			if(s1[a] == '?' and s2[a] == '?')
			{
				if(flag == 0)
					s1[a] = s2[a] = '0';
			...
			else
			{
				string t1 = s1, t2 = s2;
				t1[a+1] = t2[a+1] = 0;
				flag = strcmp(t1.c_str(), t2.c_str());
				if(flag < 0)
					flag = 1;
				else if(flag > 0)
					flag = -1;
				if(diff > 5 and a > 0 and s1_b[a-1] == '?' and s2_b[a-1] == '?')
				{
					if(flag == 1)
						s1[a-1] = min((int)'9', (char)s1[a-1]+1);
					else if(flag == -1)
						s2[a-1] = min((int)'9', (char)s2[a-1]+1);
