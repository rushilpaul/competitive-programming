...
int main()
{
	...
	while(t--)
	{
		...
		for(int a=1;a<n-1;a++)
			...
			{
				LL s=ar[a][b];
				for(int c=1;a+c<n and a-c>=0 and b+c<m and b-c>=0;c++)
				{
					s += ar[a+c][b+c] + ar[a-c][b+c] + ar[a+c][b-c] + ar[a-c][b-c];
					mx = max(mx,s);
				}
			}
		for(int a=1;a<n;a++)
			...
			{
				LL s = 0;
				for(int c=0;a+c<n and a-c-1>=0 and b+c<m and b-c-1>=0;c++)
				{
					s += ar[a+c][b+c] + ar[a-c-1][b+c] + ar[a+c][b-c-1] + ar[a-c-1][b-c-1];
					mx = max(mx,s);
				}
			}
		printf("%lld\n",mx);
