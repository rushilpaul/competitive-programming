...
{
	fact[0] = 1;
	repab(a,1,100000) fact[a] = (1ll * fact[a-1] * a) % mod;
	...
	while(t--)
	{
		...
		for(int p=n-1;p>=1;p--)
		{
			rep(j,k+1)
			{
				...
				// nCr
				LL &x = cache[k][j];
				if(x == 0) x = 1ll * fact[k] * inv(fact[j],mod) % mod * inv(fact[k-j],mod) % mod;
				ans = (ans + 1ll * pow(ar[p-1],k-j,mod) * x % mod * P[j] % mod);
				if(ans >= mod) ans -= mod;
			}
		}
		ans = ans * 2 % mod;
