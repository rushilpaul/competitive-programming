...
int main()
{
	...
	ar[n] = 100000;
	LL ans1=0, ans2=0, ans3=0, total=0;
	int F[10001], C[10001];
	memset(F,0,sizeof(int)*10001);
	memset(C,0,sizeof(int)*10001);
	repab(a,1,n-2)
	{
		F[ar[a-1]]++;
		repab(i,ar[a-1],10000)
			C[i]++;
		repab(b,a+1,n-1)
		{
			int sq = ar[b]*ar[b] - ar[a]*ar[a];
			int root = my_sqrt(sq), added;
			if(root > ar[b]) continue;
			if(root*root == sq)
			{
				if(F[root])
					ans2 += F[root];
			...
			if(added > 0)
				ans3 += added;
			//printf("a = %d, b = %d, added = %d\n",ar[a],ar[b],added);
		}
	}
	memset(C,0,sizeof(int)*10001);
	repab(a,1,n-2)
	{
		repab(i,ar[a-1],10000) C[i]++;
		repab(b,a+1,n-1)
		{
			int c = ar[b] - ar[a];
			total += (C[10000] - C[c]);
