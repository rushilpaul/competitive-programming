...
LL f(int h, char dir, LL target, LL ans)
{
	if(h == 0)
	{
		if(dir == 'L' and target == 0) return ans;
		else if(dir == 'R' and target == 1) return ans;
		else return ans;
	}
	if(target > (1LL<<(h-1)) and dir == 'R')
	{
		return f(h-1, 'R' , target-(1LL<<(h-1)), ans + (1LL<<h));
	}
	else if(target > (1LL<<(h-1)) and dir == 'L')
	{
		return f(h-1, 'R', target-(1LL<<(h-1)), ans+1);
	}
	else if(target <= (1LL<<(h-1)) and dir == 'R')
	{
		return f(h-1, 'L', target, ans+1);
	}
	else if(target <= (1LL<<(h-1)) and dir == 'L')
	{
		return f(h-1, 'L', target, ans + (1LL<<h));
	}
	else assert(0);
	return -1;
}
