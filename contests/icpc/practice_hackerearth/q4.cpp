...
int main()
{
	...
	rep(a,101) rep(b,1001) dp[a][b] = LLONG_MIN;
	
	rep(a,n)
	{
		int q = readint();
		int ar[q];
		rep(b,q) ar[b] = readint();
		LL s=0;
		LL items[m+1];
		rep(i,m+1) items[i] = LLONG_MIN;
		items[0] = 0;
		...
		rep(i,q)
		{
			...
			repab(j,i,q-1)
			{
				if(j-i+1 > m) break;
				s += ar[j];
				items[j-i+1] = max(items[j-i+1],s);
			}
		}
		rep(b,m+1)
		{
			if(items[b] < 0) continue;
			dp[a][b] = max(dp[a][b], a>0 ? dp[a-1][b] : items[b]);
			repab(w,b,m)
			{
				if(a > 0)
					dp[a][w] = max(dp[a][w], dp[a-1][w-b] + items[b]);
			}
		}
	}
	LL ans = 0;
