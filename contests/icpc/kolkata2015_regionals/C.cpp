...
int dp[1001][10001], g[1001][10001];
...
int main()
{
	...
	while(t--)
	{
		...
		rep(a,n) { A[a+1] = readint(); total += A[a+1]; }
		rep(i,n+1) rep(s,total+1) dp[i][s] = g[i][s] = 0;
		...
		repab(s,1,total)
		{
			ans = (ans + 1ll * max(2*s - total, total - 2*s) * dp[i][s] % mod);
			if(ans > mod) ans -= mod;
		}
		ans += total;
