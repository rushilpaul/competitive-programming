...
LD f(LD p)
{
	LD time = 0;
	rep(a,n)
		time = max(time, abs(ar[a].first - p) / (LD)ar[a].second);
	return time;
}

inline LL myround(long double m)
{
	return (LL)(m * 10000000ll);
}
...
int main()
{
	...
	LD L = ar[0].first, R = ar[n-1].first;
	LD last = -1;
	while(fabs(L - R) > 1e-18)
	{
		LD mid = (L + R) / 2;
		LD time1 = f(mid), time2 = f(mid + 1e-15);
		//printf("L,R = %f, %f, mid = %f, time1 = %.10f, time2 = %.10f\n",L,R,mid,time1,time2);

		//if(fabs(time2 - time1) < 1e-15)
		{
			//printf("breaking %.8f\n", time1 - time2);
			//break;
		}
