...
public class B {
    ...
    static int mirror(int n, int x) {
        return n - x - 1;
    }

    static long dist(List<Integer> L, int x) {
        long ans = 0;
        for(int i : L) ans += abs(x - i);
        return ans;
    }

    static long findMin(List<Integer> L) {

        int l = L.get(0), r = L.get(L.size() - 1);
        long bestMin = Long.MAX_VALUE;
        for(int i : L)
            bestMin = min(bestMin, dist(L, i));

        bestMin = min(bestMin, dist(L, (int) L.stream().mapToInt(i -> i).average().getAsDouble() ));
        return bestMin;
    }
        ...
        while(t-- > 0) {
            ...
            for(int i = 0; i < (n + 1) / 2; i++)
                for(int j = 0; j < (m + 1) / 2; j++) {
                    ...
                    L.add(ar[i][j]);
                    if(mirror(m, j) != j)
                        L.add(ar[i][ mirror(m, j) ]);
                    if(mirror(n, i) != i)
                        L.add(ar[mirror(n, i)][j]);

                    if(mirror(n, i) != i && mirror(m, j) != j)
                        L.add(ar[mirror(n, i)][ mirror(m, j) ]);
                    sort(L);
//                    System.out.printf("i,j: %d, %d, l = %d\n",i,j, l);
                    ans += findMin(L);
                }
            println(ans + "");
        }
