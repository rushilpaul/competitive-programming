...
public class C2 {
    ...
    static long putAndGet(int i, int x, int ar[]) {

        long sum = 0;
        sum += max(0, ar[i] - ar[i-1]);
        sum += max(0, ar[i+1] - ar[i]);
        ar[i] = x;
        long finalSum;
        finalSum = max(0, ar[i] - ar[i-1]);
        finalSum += max(0, ar[i+1] - ar[i]);
        return finalSum - sum;
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        while(t-- > 0) {
            ...
            while(q-- > 0) {
                ...
                int tmp = ar[x];
                s += putAndGet(x, ar[y], ar);
                s += putAndGet(y, tmp, ar);
                writer.println(s + "");
            }
        }
        writer.close();
