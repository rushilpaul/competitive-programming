...
public class D {
    ...
    static int ncr(int n, int r) {
        ...
        return (int) ( 1L * fact[n] * invFact[r] % mod * invFact[n - r] % mod );
    }
    ...
    public static void main(String[] args) {
        ...
        Seg[] intervals = new Seg[2 * n];

        int j = 0;
        for(int i = 0; i < n; i++) {
            int x = sc.nextInt(), y = sc.nextInt();
            intervals[j++] = new Seg(x, true);
            intervals[j++] = new Seg(y, false);
        }
        sort(intervals);

        int ans = 0, started = 0, ended = 0;

        for(int i = 0; i < j; i++) {

            if(intervals[i].start) {

                int size = started - ended;
                ans += ncr(size, k - 1);
                if (ans >= mod)
                    ans %= mod;
                started++;
    ...
    static class Seg implements Comparable<Seg> {
        ...
        public Seg(int x, boolean start) {
            this.x = x;
            this.start = start;
        }
