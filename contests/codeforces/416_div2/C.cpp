...
void init()
{
	...
	rep(a,n)
	{
		...
		repab(b,x+1,y-1)
		{
			...
			if(x1 < x or y1 > y)
			{
				L[ar[b]] = min(L[ar[b]], min(x,x1));
				R[ar[b]] = max(R[ar[b]], max(y,y1));
			}
		}
	}
	for(int a=n-1;a>=0;a--)
...
int find_ans(int i)
{
	...
	// don't take ith number
	dp = find_ans(i-1);

	// take the ith number
	int x = calcxor(i);
	if(L[ar[i]] != i) i = L[ar[i]] - 1;
	while(i >= 0)
	{
		if(R[ar[i]] > i) continue;
		dp = max(dp, x + find_ans(i));
		x ^= calcxor(i);
		i = L[ar[i]] - 1;
	}
	return dp;
...
int main()
{
	...
	init();
	rep(a,n) printf("%d ", L[ar[a]]);
	printf("\n");
	rep(a,n) printf("%d ", R[ar[a]]);
	fflush(stdout);
