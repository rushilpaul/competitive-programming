...
int main()
{
	...
	int minboy=INT_MAX, mingirl=INT_MAX, ar[2*n];
	for(int a=0;a<2*n;a++) ar[a] = readint();
	sort(ar,ar+2*n);
	mingirl = ar[0];
	minboy = ar[n];

	long double L=0,R=w;
	long double lastvalue = -1, mid = (L+R)/2.0;
	while(R-L >= 0 and abs(lastvalue-mid) > 1e-7)
	{
		lastvalue = mid;
		long double x = mid/(3.0*n);
		if(x > mingirl or 2*x > minboy)
			R = mid;
		else
			L = mid;
		mid = (L + R)/2.0;
