...
int main()
{
	...
	int ar[n][3];	// dig, letter, special
	for(int a=0;a<n;a++)
	{
		string s = readstring(100);
		ar[a][0] = ar[a][1] = ar[a][2] = INT_MAX;
		for(int i=0;i<m;i++)
			if(s[i] >= '0' && s[i] <= '9') ar[a][0] = min(ar[a][0], (i <= m/2 ? i : m-i));

		for(int i=0;i<m;i++)
			if(s[i] >= 'a' && s[i] <= 'z') ar[a][1] = min(ar[a][1], (i <= m/2 ? i : m-i));

		for(int i=0;i<m;i++)
			if(s[i] == '*' || s[i] == '&' || s[i] == '#') ar[a][2] = min(ar[a][2], (i <= m/2 ? i : m-i));
	}
		...
		for(int b=a+1;b<n;b++)
		{
			for(int c=b+1;c<n;c++)
			{
				...
				rep(k,3)
				{
					...
					if(ar[a][i] != INT_MAX and ar[b][j] != INT_MAX and ar[c][k] != INT_MAX)
						ans = min(ans, ar[a][i] + ar[b][j] + ar[c][k]);
				}
			}
		}
	}
