...
{
	string c = a+a;
	int cost = (int)c.find(b);
	if(cost == string::npos) return -1;
	return cost;
}
...
int main()
{
	...
	{
		int total = 0, flag = 1;
		//printf("Checking for target = %s\n", target.substr(a,len).c_str());
		rep(b,n)
		{
			int cost = calc(ar[b], target.substr(a,len));
			//printf("cost for %s: %d\n", ar[b].c_str(),cost);
			if(cost == -1) { flag = 0; break; }
			total += cost;
		}
