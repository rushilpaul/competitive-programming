...
int main()
{
	...
	while(tt--)
	{
		LL x = readint(), y = readint(), p = readint(), q = readint();
		if((x == 0 and p == 0) or (x == y and p == 1 and q == 1)) { printf("0\n"); continue; }
		LL c = x * q - y * p;
		LL x0, y0;

		egcd(p,q,x0,y0);
		y0 = -y0;
		//printf("x0 y0 %lld, %lld\n", x0, y0);
		...
		LL t = min( floor((LD)c * (x0 - y0) / (q-p)), min( floor((LD)c * x0 / q), floor((LD)c * y0 / p)));
		//printf("t = %lld\n", t);
		LL j = c * x0 - q * t;
