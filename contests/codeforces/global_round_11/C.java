...
public class C {
    ...
    public static void main(String[] args) {
        ...
        int r = sc.nextInt() * 2, n = sc.nextInt();
        int x[] = new int[n+1];
        int y[] = new int[n+1];
        x[0] = 1;
        y[0] = 1;
        ...
        int maxAns[] = new int[n + 1];
        int dp[] = new int[n + 1];
        maxAns[0] = dp[0] = 1;

        for(int i = 1; i <= n; i++) {

            int max = i - r >= 0 ? dp[i - r] : 0;
            for(int j = i-1; j >= max(0, i - r); j--) {
                if(abs(x[i] - x[j]) + abs(y[i] - y[j]) + times[j] <= times[i])
                    max = max(max, maxAns[j]);
            }
            maxAns[i] = max == 0 ? 0 : max + 1;
            dp[i] = max(dp[i-1], maxAns[i]);
        }
        println(max(maxAns) - 1);
    }
