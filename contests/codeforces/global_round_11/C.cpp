...
int dp[MAX_SIZE];
int maxVal[MAX_SIZE], times[MAX_SIZE], x[MAX_SIZE], y[MAX_SIZE];

int main()
{
	int r = readint() * 2, n = readint();
	for(int i = 1; i <= n; i++) {
		times[i] = readint();
		x[i] = readint();
		y[i] = readint();
	...
	for(int i=1; i<=n; i++) {
		...
		int maxSoFar = i-r >= 0 ? dp[i-r] : 0;
		for(int j=i-1; j>=max(0, i-r); j--) {

			if(abs(x[j] - x[i]) + abs(y[j] - y[i]) + times[j] <= times[i])
				maxSoFar = max(maxSoFar, maxVal[j]);
		}
		maxVal[i] = maxSoFar == 0 ? 0 : (maxSoFar + 1);
		dp[i] = max(dp[i-1], maxVal[i]);
	}
	printf("%d\n", dp[n] - 1);
	return 0;
}
