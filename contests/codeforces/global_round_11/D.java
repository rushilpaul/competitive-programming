...
public class D {
    ...
    static List<Op> operations = new ArrayList<>();

    static void shuffle(int ar[], List<Integer> splits) {
        ...
        for(int i = splits.size() - 2; i >= 0; i--) {
            int start = splits.get(i) + 1;
            int part = 0;
            while(start <= splits.get(i+1)) {
                newArr[k++] = ar[start++];
                part++;
            }
            if(part > 0)
    ...
    static int indexOf(int ar[], int x) {
        int n = ar.length;
        for(int i = 0; i < n; i++)
            if(ar[i] == x)
                return i;
    ...
    public static void main(String[] args) throws Exception {
        ...
        while(true) {
            ...
            for(int i = 1; i < n; i++) {
                int posLower = indexOf(ar, i);
                int posHigher = indexOf(ar, i+1);

                if(posLower > posHigher) {
                    done = false;
                    if(posHigher - 1 >= 0)
                        splits.add(posHigher - 1);
                    if(posLower < n-1)
                        splits.add(posLower);

                    int j;
                    for(j = posLower; j >= posHigher; j--)
                        if(ar[j] != ar[j-1] + 1)
                            break;
                    j--;
                    if(!splits.contains(j) && j >= 0)
                        splits.add(j);
                    break;
                }
            }
        ...
        writer.println(operations.size());
        for(Op op : operations) {
            writer.print(op.sizes.size() + " ");
            for(int size : op.sizes)
                writer.print(size + " ");
            writer.println("");
        }
        writer.close();
