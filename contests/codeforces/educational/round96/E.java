...
public class E {
    ...
    static long merge(int A[], int i, int j) {
    ...
    static long mergeSort(int ar[], int i, int j) {
        if(i == j)
            return 0;
        int mid = (i + j) / 2;
        long s1 = mergeSort(ar, i, mid);
        long s2 = mergeSort(ar, mid + 1, j);
        return merge(ar, i, j) + s1 + s2;
    }
    ...
    public static void main(String[] args) {
        ...
        int ar[] = new int[n];
        LinkedList<Integer> pos[] = new LinkedList['z' + 1];
        ...
        for(int i = 0; i < n; i++) {
            char ch = s[n-1-i];
            ar[i] = pos[ch].removeFirst();
        }
        println(mergeSort(ar, 0, n-1));
    }
