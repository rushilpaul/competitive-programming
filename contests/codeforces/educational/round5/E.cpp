...
inline LL f(LL i) { return i % mod * ((i+1) % mod) % mod * inv(2,mod) % mod; }
...
int main()
{
	...
	LL end = min((LL)sqrt(n), m);
	for(LL a=1;a<=end;a++) ans = (ans - n/a*a) % mod;
	LL x = n / end - 1;
	for(LL a=n/m;a<=x;a++)
	{
		LL i = n/(a+1)+1, j = min(n/a,m);
		if(i > j) continue;
		//printf("a: %lld, %lld %lld\n",a,i,j);
		ans = (ans - a % mod * (i % mod * ((j-i+1) % mod) % mod + f(j-i)) % mod) % mod;
	}
	if(ans < 0) ans += mod;
	cout << ans << endl;
	return 0;
