...
public class B {
    ...
    public static void main(String[] args) {
        ...
        while(t-- > 0) {
            ...
            if(np >= 5) {
                ans = Math.max(ans, getMaxProd(pos, neg, 5, true));
            }
            if(nn >= 2 && np >= 3) {
                ans = Math.max(ans, getMaxProd(pos, neg, 3, true) * getMaxProd(pos, neg, 2, false));

            }
            if(nn >= 4 && np >= 1) {
                ans = Math.max(ans, getMaxProd(pos, neg, 1, true) * getMaxProd(pos, neg,4, false));
            }
            if(nn >= 5) {
                ans = Math.max(ans, getMinProd(pos, neg, 5, false));
            }
    ...
    static long getMaxProd(int pos[], int neg[], int x, boolean isPos) {
        long ans = 1;
        int prev = x;
        for(int i = 3000; i >= 0; i--) {
            if(isPos && pos[i] > 0 && x > 0) {
                int diff = Math.min(x, pos[i]);
                x -= diff;
                while(diff > 0) {
                    diff--;
                    ans *= i;
            ...
            if(!isPos && neg[i] > 0 && x > 0) {
                int diff = Math.min(x, neg[i]);
                x -= diff;
                while(diff > 0) {
                    diff--;
                    ans *= i;
    ...
    static long getMinProd(int pos[], int neg[], int x, boolean isPos) {
        long ans = 1;
        int prev = x;
