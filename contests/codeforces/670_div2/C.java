...
public class C {
    ...
    static int sizes[], n, par[];

    static int dfs(int u, int parent, List<Integer> G[]) {
        ...
        sizes[u] = Math.max(max, n - s);
        return s;
    }

    static int getNode(int u, List<Integer> G[]) {
    ...
    public static void main(String[] args) {
        ...
        while(t-- > 0) {
            n = sc.nextInt();
            List<Integer> G[] = sc.adjacencyList(n, n - 1);
            par = new int[n + 1];
            sizes = new int[n + 1];
            dfs(1, 0, G);
            sizes[0] = Integer.MAX_VALUE;
            int cent = min(sizes);
            ...
            if (centroids.size() > 2)
                ...
                int c1 = -1, c2 = -1;
                for (int i = 1; i <= n; i++) {
                    if (sizes[i] == cent && c1 == -1)
                        c1 = i;
                    else if (sizes[i] == cent && c2 == -1)
                        c2 = i;
                }
                dfs(c1, 0, G);
                int other = getNode(c2, G);
                System.out.printf("%d %d\n", other, par[other]);
                System.out.printf("%d %d\n", c1, other);
            }
        }
