...
int main()
{
	LL n = readint(), l = readint(), r = readint();
	if(n == 0) { printf("0\n"); return 0; }
	vector<LL> ar;
	while(n > 1)
	{
	...
	for(LL x : ar)
	{
		k *= 2;
		if(x % 2 == 0) continue;
		LL i1 = (l - k) / (2*k), i2 = (r - k) / (2*k);

		if(l-k < 0 and r-k >= 0) i1 = -1;
		else if(l-k < 0 and r-k < 0) i1 = i2 = -1;
		ans += (i2 - i1);

		if(l-k < 0 and r-k < 0) continue;
		else if( (l-k) % (2*k) == 0) ans++;
	}
	printf("%lld\n",ans);
