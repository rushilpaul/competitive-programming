...
int in(pair<int,int> p, int n) { return p.first == n or p.second == n ? n : 0; }

int overlap(pair<int,int> p1, pair<int,int> p2)
{
	if(in(p1, p2.first))
		return p2.first;
	else if(in(p1, p2.second))
		return p2.second;
	return 0;
}

pair<int,int> successors[200001];
vector<int> ans;
...
int main()
{
	...
	{
		ans.pb(i);
		auto p1 = successors[i];
		auto p2 = successors[p1.first];
		int o = overlap(p1, p2);
		if(o == p1.second)
			i = p1.first;
		else
