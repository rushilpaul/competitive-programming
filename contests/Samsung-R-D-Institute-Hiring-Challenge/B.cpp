...
void printBest(int n, int m)
{
	repab(a,1,n)
	{
		repab(b,1,m)
			printf("{%d %d} ", best[a][b].first, best[a][b].second);
		printf("\n");
	}
}
...
int main()
{
	...
	for(int i = n; i >= 1; i--)
		for(int j = m; j >= 1; j--)
			...
			{
				if(j != m)
					D[i][j] = (D[i][j] + (!compo[A[i][j+1]] ? D[i][j+1] : 0)) % mod;

				if(i != n)
					D[i][j] = (D[i][j] + (!compo[A[i+1][j]] ? D[i+1][j] : 0)) % mod;

				if(i != n and j != m)
					D[i][j] = (D[i][j] + (!compo[A[i+1][j+1]] ? D[i+1][j+1] : 0)) % mod;
			}
	...
	for(int j = m; j >= 1; j--)
		...
		{
			if(j != m)
				best[i][j] = !compo[A[i][j+1]] and D[i][j+1] ? mp(i,j+1) : best[i][j];

			if(i != n)
				best[i][j] = !compo[A[i+1][j]] and D[i+1][j] ? mp(i+1,j) : best[i][j];

			if(i != n and j != m)
				best[i][j] = !compo[A[i+1][j+1]] and D[i+1][j+1] ? mp(i+1,j+1) : best[i][j];
		}

	printf("%d\n", D[1][1]);
	pair<int,int> cur = {1,1};
	while(D[1][1] and cur.first and cur.second)
	{
		printf("%d %d\n", cur.first, cur.second);
		cur = best[cur.first][cur.second];
	}
