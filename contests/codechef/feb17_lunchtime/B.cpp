...
int minx = -1000, maxx = 1000, miny = 0, maxy = 1000;
...
int inside(int x, int y)
{
	...
	char s[100];
	scanf("%s",s);
	return s[0] == 'Y' and s[1] == 'E' and s[2] == 'S';
}
...
int main()
{
	int top = find_topmost();
	int sq_right = find_rightmost(0);
	int sq_area = (sq_right*2) * (sq_right*2);
	int t_right = find_rightmost(sq_right*2);
	int t_base = t_right * 2;
	int t_height = top - (sq_right*2);
	printf("! %d\n", sq_area + t_base * t_height / 2);
