...
matrix I(int n)	// identity matrix of size n
{
...
int main()
{
	...
	while(t--)
	{
		...
		matrix Me(m,vector<LL>(m,0)), Mo(m,vector<LL>(m,0));
		for(int i=0;i<m;i++)
		{
			if(i-1 >= 0) Me[i][i-1] = Mo[i][i-1] = 1;
			if(i+1 < m) Me[i][i+1] = Mo[i][i+1] = 1;
			Mo[i][i] = 1;
		}
		n--;
		matrix res = mul(pow(Me,(n+1)/2), pow(Mo,n/2));
		LL ans = 0;
		for(int i=0;i<m;i++)
			for(int j=0;j<m;j++)
				ans = (ans + res[i][j]) % mod;
