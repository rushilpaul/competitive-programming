...
int main()
{
	...
	rep(a,n)
		ans += abs(x[a].first) + abs(x[a].second);
	...
	rep(a,m)
	{
		if(s[a] == 'L' or s[a] == 'R')
		{
			pair<int,int> cur = {curx, 100000000};
			if(s[a] == 'L') cur.second = -100000000;
			int p = lower_bound(all(x), cur) - x.begin();
		...
		{
			pair<int,int> cur = {cury, 100000000};
			if(s[a] == 'D') cur.second = -cur.second;
			int p = lower_bound(all(y), cur) - y.begin();
