...
LL finda(LL n, int P, int base)
{
	...
	for(int a = 1; a <= (n % P); a++)
		if(gcd(a, base) == 1)
			...
			fact = fact * removePrime(a,base) % P;
...
int main()
{
	...
	while(t--)
	{
		...
		int M = readint();
		LL n = (N+K-1)/K, r = n-1, numParts = n;
		n = n + n*K - N - 1;
		printf("n = %lld, r = %lld\n", n, r);

		auto primes = getPrimeFactors(M);
		vector< pair<int,int> > v(primes.size());
		for(int a=0;a<v.size();a++)
		{
			int mi = pow(primes[a].first, primes[a].second), Mi = M / mi;
			int x,y;
			egcd(Mi,mi,x,y);
			v[a] = {Mi,x};
		}
		...
		rep(a, primes.size())
		{
			int base = primes[a].first, k = primes[a].second;
			int P = pow(base,k);
			printf("P = %d\n", P);
			LL k2 = f(n, base) - f(r, base) - f(n-r, base);
			if(k2 >= k)
				continue;
			...
			for(int a = 1;a <= (n % P); a++)
				if(gcd(a,base) == 1)
					...
					rem = rem * removePrime(a,base) % P;

			rem = rem * finda(r, P, base) % P;
			rem = rem * finda(n-r, P, base) % P;
			rem = rem * pow(base, k2, P) % P;

			printf("Mi = %d, Ni = %d, a = %lld\n", v[a].first, v[a].second, rem);
			ans = (ans + rem * v[a].first % M * v[a].second) % M;
		}
		if(ans < 0) ans += M;
		printf("%lld %lld\n", numParts, ans);
	}
