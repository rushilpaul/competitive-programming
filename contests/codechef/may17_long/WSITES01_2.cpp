...
set<int> goodhashes, badhashes;
set<string> ans;
 
LL getHash(string s, int n, set<int> &hashes, int good)
{
	LL hash = 0, prime = 41, p = 1, mod = 2016634091l;
	string prefix = "";
	rep(a,n)
	{
		hash = (hash + p * s[a]) % mod;
		...
		{
			prefix += s[a];
			if(goodhashes.find(hash) == goodhashes.end())	// if prefix of bad string is not a prefix of any other good string
			{
				ans.insert(prefix);
				return hash;
			}
...
int main()
{
	...
	for(string s : good)
		getHash(s, s.length(), goodhashes, 1);
	for(string s : bad)
		if(getHash(s, s.length(), badhashes, 0) == -1)
		{
			printf("-1\n");
			return 0;
		}
