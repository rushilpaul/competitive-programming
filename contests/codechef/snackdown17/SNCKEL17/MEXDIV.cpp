...
int main()
{
	...
	for(int a=2;a<=n;a++)
	{
		...
		while(m.size() == k+1)
		{
			if(m.find(ar[left]) != m.end())
				m[ar[left]] = m[ar[left]] - 1;
			if(m[ ar[left] ] == 0) m.erase(m.find(ar[left]));
			left++;
		}
		ans[a] = (C[a-1] - (left == 1 ? -1 : C[left-2])) % mod;
		C[a] = (C[a-1] + ans[a]) % mod;
	}
