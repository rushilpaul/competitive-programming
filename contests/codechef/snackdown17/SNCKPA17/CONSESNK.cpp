...
LL f(int n, int L, int start)
{
	//printf("for p = %d, ", start);
	LL ans = 0, cur = start;
	rep(a,n)
	{
		ans += abs(cur - ar[a]);
...
int main()
{
	...
	while(t--)
	{
		...
		sort(ar,ar+n);
		int min_p = A, max_p = B - n*L;
		int p1 = -1, p2 = -1;

		while(min_p < max_p)
		{
			p1 = min_p; p2 = max_p;
			int mid = (min_p + max_p) / 2;
			//printf("%d %d\n", min_p, max_p);
			if(f(n,L,mid) <= f(n,L,mid+1))
				max_p = mid;
			else
				min_p = mid;
			if(p1 == min_p and p2 == max_p) break;
		}
		printf("%lld\n", min(f(n,L,min_p), f(n,L,max_p)));
	}
	return 0;
}
