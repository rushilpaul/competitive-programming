...
int n, total;
int dp[4][2][512][502];	// dir, p, i, number of blocks

// dir
// 0 top, 1 bot, 2 left, 3 right

int f(string s[2], int p, int i, int T, int dir)
{
	if(i < 0 or i >= n or s[p][i] == '.') return total == T;
	if(s[p][i] == '*') return 0;
	//printf("i = %d, p = %d, dir = %d, T = %d\n", i, p, dir, T);

	int &ret = dp[dir][p][i][T];
	if(ret >= 0)
		return ret;
