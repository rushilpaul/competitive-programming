...
int main()
{
	...
	while(t--)
	{
		...
		//rep(a,n) printf("f[ %d ] = %d\n", ar[a], F[a]);

		while(q--)
		{
			int k; scanf("%d",&k);
			int id = lower_bound(ar,ar+n,k) - ar;
			if(ar[id] >= k and id >= 0) id--;

			if(id <= 0 and ar[id] >= k) printf("%d\n",n);
			else
			{
				if(id >= n) id = n-1;
				//printf("id = %d, num = %d\n", id, ar[id]);
				int cap = id+1, ans = n - id - 1;
				do
				{
					int rem = k - ar[id], c = F[id];
					assert(rem >= 0);
					int cp = cap / (rem + 1);
					cp = min(cp, c);
					//printf("cp = %d, id = %d\n", cp,id);
