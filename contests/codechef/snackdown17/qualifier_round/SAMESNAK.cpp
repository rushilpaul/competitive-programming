...
int in(int x, int a, int b) { if(a > b) swap(a,b); return x >= a and x <= b; }
...
int main()
{
	...
	while(t--)
	{
		...
		x12 = readint();
		y12 = readint();
		int x_axis1 = y11 == y12;	// parallel to x-axis

		x21 = readint();
		y21 = readint();
		x22 = readint();
		y22 = readint();
		int x_axis2 = y21 == y22;	// parallel to x-axis
		
		int yes;
		if(x_axis1 == x_axis2)	// if parallel
		{
			if(x_axis1)
			{
				yes = y11 == y21 and (in(x11, x21, x22) or in(x12, x21, x22));
				yes = yes or (y11 == y21 and (in(x21, x11, x12) or in(x22, x11, x12)));
			}
			else
			{
				yes = x11 == x21 and (in(y11, y21, y22) or in(y12, y21, y22));
				yes = yes or (x11 == x21 and (in(y21, y11, y12) or in(y22, y11, y12)));
			}
		}
		else
		{
			yes = (x11 == x21 and y11 == y21) or (x11 == x22 and y11 == y22) or (x12 == x21 and y12 == y21) or (x12 == x22 and y12 == y22);
			yes = yes or (x_axis1 and x11 == x12 and x11 == x21 and in(y11, y21, y22)) or (x_axis2 and x21 == x22 and x21 == x11 and in(y21, y11, y12));
		}
