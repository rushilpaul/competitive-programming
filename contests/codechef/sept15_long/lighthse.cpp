...
int main()
{
	...
	while(t--)
	{
		...
		for(int a=0;a<n;a++)
		{
			if(P[a].x == minx && P[a].y == miny)
			{
				printf("1\n%d NE\n",a+1);
				flag = 1;
				break;
			}
			if(P[a].x == minx && P[a].y == maxy)
			{
				printf("1\n%d SE\n",a+1);
				flag = 1;
				break;
			}
			if(P[a].x == maxx && P[a].y == miny)
			{
				printf("1\n%d NW\n",a+1);
				flag = 1;
				break;
			}
			if(P[a].x == maxx && P[a].y == maxy)
			{
				printf("1\n%d SW\n",a+1);
				flag = 1;
				break;
		...
		for(int a=0;a<n;a++)
		{
			if(P[a].x == minx && p1 < 0)
				p1 = a;
			else if(P[a].x == maxx && p2 < 0)
				p2 = a;
		}
		printf("2\n%d %s\n%d %s\n",p1+1, (P[p1].y <= P[p2].y ? "NE" : "SE"), p2+1, (P[p2].y < P[p1].y ? "NW" : "SW"));
	}
	return 0;
}
