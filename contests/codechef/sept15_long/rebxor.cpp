...
inline void insert(node *root, int n, int i)
{
	while(i>=0)
	{
		if(n&(1<<i))
...
int find(node *root, int n, int i)
{
	int ans=0;
	while(i >= 0 and root)
	{
...
int main()
{
	...
	for(int a=0;a<n;a++)
	{
		cur ^= ar[a];
		F[a] = cur;
		int c = find(root, cur, 29);
		F[a] = max(F[a],c);
		if(a>0)
			F[a] = max(F[a],F[a-1]);
		insert(root,cur,29);
	}
	node *root2 = &buf[buf_i++];
	cur = ans = 0;
	for(int a=n-1;a>=0;a--)
	{
		cur ^= ar[a];
		B[a] = cur;
		int c = find(root2,cur,29);
		B[a] = max(B[a],c);
		if(a<n-1)
			B[a] = max(B[a],B[a+1]);
		insert(root2,cur,29);
