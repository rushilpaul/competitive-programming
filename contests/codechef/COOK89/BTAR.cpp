...
int main()
{
	...
	{
		int n = readint();
		int ar[4]; ar[0] = ar[1] = ar[2] = ar[3] = 0;
		int s = 0, sub = 0;
		rep(a,n)
		{
			int i = readint();
		...
		int c13 = min(ar[1], ar[3]);
		ar[1] -= c13;
		ar[3] -= c13;

		int c22 = max(0,ar[2]) / 2;
		ar[2] -= (c22 * 2);

		int c211 = min(max(0,ar[2]) * 2, max(0,ar[1]) / 2 * 2) / 2;
		ar[2] -= c211;
		ar[1] -= (c211 * 2);

		int c233 = min(max(0,ar[2]) * 2, max(0,ar[3]) / 2 * 2) / 2;
		ar[2] -= c233;
		ar[3] -= c233 * 2;

		int c3333 = max(0,ar[3]) / 4;
		ar[3] -= c3333 * 4;
	
		int c1111 = max(0,ar[1]) / 4;
		//printf("c13 = %d, c22 = %d, c211 = %d, c233 = %d, c1111 = %d\n", c13, c22, c211, c233, c1111);
		printf("%d\n", sub - (c13 + c22 + c211 + c233 + c3333 + c1111));
	}
	return 0;
}
