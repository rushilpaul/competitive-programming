...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while(t-- > 0) {
           ...
           for(int i = 0; i < n; i++) {
               boolean isCop = gcd(ar[i], ar[i+1]) == 1;
               if(isCop) {
                   if(s > 0) {
                       L.add(s);
                       if(s == i)
    ...
    static int f(List<Integer> L1, List<Integer> L2, int n1, int n2) {

        int max = 0;
        for(int i = 0; i < L1.size() && n1 > 0; i++) {
            max += L1.get(i);
            n1--;
        }
        for(int i = 0; i < L2.size() && n2 > 0; i++) {
            if(n2 >= 2)
                max += L2.get(i);
            n2 -= 2;
        }
    ...
    static class FastWriter {
        ...
        void println() throws Exception { writer.newLine(); }
    ...
    static void println(BufferedWriter writer, List arr) throws IOException {
        for(Object e: arr) writer.write(e + " ");
        writer.write("\n");
    }
