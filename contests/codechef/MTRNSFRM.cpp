...
vector<int> ar[100001], A[100001], B[100001];
int n,m;
...
int main()
{
	...
	while(t--)
	{
		...
		rep(i,n)
		{
			...
			{
				ar[i][j] = A[i][j] - B[i][j];
				if(ar[i][j] + ar[0][0] - ar[i][0] - ar[0][j] != 0)
					flag = 0;
			}
		}
		if(flag == 0)
		...
		LL L = INT_MIN, R = INT_MAX, ans, ans_1;
		while(L <= R)
		{
			int x = (L + R) / 2;
			ans = f(x);
			...
			//printf("L,R = %lld, %lld, ans,ans_1: %lld, %lld, x = %d\n", L, R, ans, ans_1, x);
		}
		printf("%lld\n", min(ans, ans_1));
	}
	return 0;
