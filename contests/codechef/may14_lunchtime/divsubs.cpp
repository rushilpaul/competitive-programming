...
using namespace std;
int ar[100000], vec[100000][2], dup[100000];
...
int main()
{
	...
	while(t--)
	{
		int n = readint();
		int s = 0;
		memset(dup,0,sizeof(int)*n);
		for(int a=0;a<n;a++)
			ar[a] = readint();
		for(int a=0;a<n;a++)
		{
			s = (s+ar[a])%n;
			if(dup[s] == 1) { vec[s][1] = a; break; }
			dup[s]=1;
			vec[s][0] = a;
			if(s == 0) { vec[s][0] = -1; vec[s][1] = a; break; }
		}
		printf("%d\n",vec[s][1]-vec[s][0]);
		for(int a=vec[s][0]+1;a<=vec[s][1];a++)
			printf("%d ",a+1);
		printf("\n");
	}
	return 0;
