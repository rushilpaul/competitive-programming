...
int main()
{
	...
	while(t--)
	{
		...
		for(int a=1;a<=MAX;a++)
		{
			if(!f && P[a].size() == 1)	// single
			{
				C1.push_back( {a, P[a][0]} );
				P[a][0] = -1;	// mark it by its position
				f = 1;
			}
			else if(P[a].size() == 2)
			{
		...
		// Shift C1's position
		int temp = C1[0].second;
		rep(a, C1.size()-1)
			C1[a].second = C1[a+1].second;
		C1[C1.size()-1].second = temp;

		for(int a=1;a<=MAX;a++)
		{
			if(P[a].size() == 1 and P[a][0] != -1)	// single
				C2.push_back( {a, P[a][0]} );
			else if(P[a].size() == 2)
				C2.push_back( {a, P[a][1]} );
		}
		...
		temp = C2[0].second;
		rep(a, C2.size()-1)
			C2[a].second = C2[a+1].second;
		C2[C2.size()-1].second = temp;
