...
int main()
{
	...
	while(t--)
	{
		...
		for(int a=0;a<n-1;a++)
		{
			...
			{
				v.clear();
				marked.insert(ar[a]); marked.insert(ar[a+1]);
				vector<int>& first = m[ar[a]], second = m[ar[a+1]];
				int i = 0, j = 0;
				while(i < first.size() and j < second.size())
				{
					if(first[i] < second[j]) { v.pb(ar[a]); i++; }
					else { v.pb(ar[a+1]); j++; }
				}
				while(i < first.size())
				{
				...
				dp[0] = {0,1};
				pair<LL,LL> e1(0,0), e2(0,0);
				if(v[0] == ar[a]) e1.second = 1; else e2.second = 1;

				repab(i,1,size-1)
				{
					if(v[i] == ar[a])
					{
						dp[i] = {e2.second, e2.first + 1};
						e1.first += e2.second;
						e1.second += e2.first + 1;
						if(e1.first > mod) e1.first -= mod;
						if(e1.second > mod) e1.second -= mod;
					}
					else
					{
						dp[i] = {e1.second, e1.first + 1};
						e2.first += e1.second;
						e2.second += e1.first + 1;
						if(e2.first > mod) e2.first -= mod;
						if(e2.second > mod) e2.second -= mod;
					}
				}
				for(auto e : dp)
				{
		...
		for(int a=0;a<n;a++)
			if(marked.find(ar[a]) == marked.end())
			{
				ans++;
				if(ans > mod) ans -= mod;
			}
