...
int main()
{
	clock_t tm = clock();
	//pre();
	int ar[] = {2,4,9,16,25,64,289,729,1681,2401,3481,4096,5041,7921,10201,15625,17161,27889,28561,29929,65536,83521,85849,146689,262144,279841,458329,491401,531441,552049,579121,597529,683929,703921,707281,734449,829921,1000000};
	int n = sizeof(ar)/sizeof(int);
	int t = readint();
	while(t--)
	{
		int A = readint(), B = readint();
		int i = lower_bound(ar,ar+n,A) - ar, j = lower_bound(ar,ar+n,B) - ar;
		if(ar[j] != B) j--;
		writeint(j-i+1);
		putchar('\n');
	}
	//tm = clock() - tm;
	//writedouble((double)tm/CLOCKS_PER_SEC,2);
	flush();
	return 0;
}
